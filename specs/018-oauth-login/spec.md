...
- **FR-013**: The CLI MUST support a hybrid OAuth client model: a default project-owned **public OAuth application** (PKCE-only, no client secret) whose `client_id` is shipped with the released binary as the out-of-the-box experience, AND an opt-in override (env var or config key) that lets a user / org supply their own self-registered OAuth application's `client_id` (and redirect URI) so installations on locked-down tenants can avoid the shared app. Resolved on issue #37 by the owner on 2026-04-27.
- **FR-013a**: Because the project-owned `client_id` is shipped publicly in the released binary, the OAuth flow MUST treat the `client_id` as **non-secret** (per OAuth public-client convention). Security MUST come from: (a) PKCE on every authorization request, (b) strict validation of the redirect URI on the local callback (loopback only, exact match against the value sent on the authorization request), (c) validation of the OAuth `state` parameter to bind the callback to the originating session, and (d) least-privilege OAuth scopes. The CLI MUST NEVER embed an OAuth client secret in the released binary.
- **FR-014**: When a stored OAuth credential's refresh path fails for non-network reasons (refresh token rejected / revoked, refresh window exceeded, organization access removed, app disabled on the tenant), the CLI MUST surface a clear, actionable error naming the affected organization and instructing the user to run an explicit re-login (e.g. `azdo login --org <O>`), and MUST leave the existing stored credential in place so the user can inspect or report it. The CLI MUST NOT silently delete the dead credential and silently re-prompt — the failure mode often signals an account-side condition (revocation, removal) the user needs to be aware of. Resolved on issue #37 by the owner on 2026-04-27.
- **FR-015**: The repository MUST ship a clear markdown guide that walks a reader through registering the Azure DevOps OAuth application end-to-end (which AzDO / Entra portal to use, the redirect URI to set, the scopes to grant, where to find the resulting `client_id`, and any tenant-policy gotchas). The guide MUST serve two audiences: (a) the project maintainer producing the shared `client_id` shipped in the released binary (so the maintainer can hand the value off to the implementation), and (b) end users on locked-down tenants who need to register their own OAuth app for the FR-013 override path. The guide MUST be written so a reader who is unfamiliar with AzDO app registration can complete the registration on the first attempt.
- **FR-016**: The OAuth flow MUST request **least-privilege scopes** that exactly mirror the published PAT scope table (FR-008): Work Items read/write and Code read for Pull Requests as the initial baseline, plus any additional minimum scopes the CLI's existing capability surface requires. The PAT scope table is the single source of truth for both authentication paths. When a future CLI feature needs an additional OAuth scope, it MUST be added to the published PAT scope table first, and existing OAuth users MUST re-consent rather than the CLI silently widening the requested scope set. The CLI MUST NOT request `vso.full_access` (or any equivalent broad scope) by default. Resolved on issue #37 by the owner on 2026-04-27.
...
- Q: Which Azure DevOps OAuth client model is in scope — single shared public app, per-user-registered app, or hybrid? → A: Hybrid (option C). Default to a project-owned public OAuth app (PKCE, no client secret) whose client id ships in the released binary; allow an env-var / config override so users on locked-down tenants can supply their own client id. The shared client id is intentionally non-secret per OAuth public-client convention; security comes from PKCE, strict loopback redirect-URI validation, the OAuth `state` binding, and least-privilege scopes. NEVER embed a client secret in the released binary. [owner: alkampfergit, 2026-04-27]
- Q: When a stored OAuth credential's refresh path fails (token revoked / refresh window exceeded / org access removed), should the CLI silently delete the dead credential and re-prompt, OR surface the error and require explicit re-login? → A: Surface the error and require explicit re-login (option B, "Continue"-accepted recommendation). Refresh failure is a signal the user needs to see; silent re-auth would mask account-side conditions like revocation. The dead credential stays in place until the user runs `azdo login` / `azdo logout` themselves. [owner: alkampfergit, 2026-04-27]
- Q: Beyond the spec itself, the project also needs an external artefact to register the AzDO OAuth app. → A: Ship a clear markdown guide (FR-015) covering how to register the AzDO OAuth application end-to-end. Two audiences: (a) the project maintainer who needs to produce the shared `client_id` shipped in the released binary, (b) end users on locked-down tenants who use the FR-013 override path with their own self-registered app. [owner: alkampfergit, 2026-04-27]
- Q: What OAuth scope policy should the CLI use — mirror the PAT scope table, request a broad scope like `vso.full_access`, or default-narrow with an opt-in flag? → A: Mirror the documented PAT scope table (option A). The PAT scope table (FR-008) is the single source of truth for both PAT and OAuth scopes; new feature scopes are added to the table first and existing OAuth users re-consent rather than the CLI silently widening scope. No `vso.full_access` by default. Codified in FR-016. [owner: alkampfergit, 2026-04-27]
