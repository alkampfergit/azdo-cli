...
- **FR-007**: The CLI MUST continue to accept Personal Access Token authentication as a deliberate, supported alternative to OAuth — existing PAT users MUST NOT be force-migrated and MUST be able to opt into PAT on a fresh install.
...
- **FR-012**: [NEEDS CLARIFICATION: Should the OAuth login command become the new default for `azdo login`, with PAT available via an explicit `--pat` flag, OR should the user explicitly opt into the OAuth flow (e.g. `azdo login --oauth`) while PAT remains the default during a transition period?]
