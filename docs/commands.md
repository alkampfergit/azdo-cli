...
| `azdo auth token` | Print the token the CLI uses for an org on stdout, for API calls the CLI does not wrap (no `--json` — see [docs/authentication.md](authentication.md#exporting-the-token)) | `--org` |
